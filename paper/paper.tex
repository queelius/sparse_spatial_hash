...
\author{
DigiStar Contributors\\
\texttt{github.com/spinoza/sparse\_spatial\_hash}
}
...
\item Targeted optimizations including precomputed division reciprocals, manual loop unrolling, and optimized Morton encoding achieving 23\% overall speedup
\item Performance evaluation demonstrating competitive throughput: 1.8M entities/second build, 400K queries/second, 2.4M pairs/second collision detection
\end{enumerate}
...
The implementation uses \texttt{std::unordered\_map} as the underlying hash table, mapping cell coordinates to vectors of entity indices. This design choice balances several factors:
...
Cell buckets use \texttt{std::vector} to store entity indices. While this introduces heap allocations for each occupied cell, typical bucket sizes of 2-5 entities keep memory overhead acceptable. Future optimization could employ small vector optimization (SVO) to store small buckets inline.
...
\begin{table}[h]
\centering
\caption{Grid build performance (3D, bounded topology)}
\label{tab:build}
\begin{tabular}{lrrr}
\toprule
Entity Count & Time & Throughput & Per-Entity \\
\midrule
1,000 & 109 $\mu$s & 9.2M entities/s & 109 ns \\
10,000 & 679 $\mu$s & 14.7M entities/s & 68 ns \\
100,000 & 5.63 ms & 17.8M entities/s & 56 ns \\
\bottomrule
\end{tabular}
\end{table}

Throughput increases with entity count due to amortization of hash table initialization costs and better cache utilization from spatial locality. The 100K entity case achieves 17.8M entities/second throughput, or approximately 56 nanoseconds per entity including hash computation, bucket insertion, and memory allocation.
...
\begin{table}[h]
\centering
\caption{Update performance (10K entities, 1\% cell changes)}
\label{tab:update}
\begin{tabular}{lrr}
\toprule
Method & Time & Speedup \\
\midrule
Full rebuild & 679 $\mu$s & 1.0x \\
Incremental update & 17 $\mu$s & 39.9x \\
\bottomrule
\end{tabular}
\end{table}

Incremental updates achieve nearly 40x speedup when only 1\% of entities change cells, matching the theoretical expectation. This optimization is critical for real-time applications maintaining 60 FPS where frame budgets are 16.7 milliseconds.
...
\begin{table}[h]
\centering
\caption{Topology mode comparison (10K entities)}
\label{tab:topology}
\begin{tabular}{lr}
\toprule
Topology & Time \\
...
\midrule
\textbf{Geometric mean} & \textbf{23\%} \\
\bottomrule
\end{tabular}
\end{table}

The geometric mean improvement of 23\% across all benchmarks demonstrates the effectiveness of targeted micro-optimizations when applied to hot code paths.
...
\begin{itemize}
\item 17.8M entities/second build throughput for 100K entities
\item 40x speedup for incremental updates with 1\% cell changes
...
The design leverages C++20 concepts, compile-time polymorphism, and position accessor traits to provide zero-overhead abstractions while maintaining extensibility and type safety. Targeted optimizations including precomputed reciprocals, manual loop unrolling, and optimized Morton encoding achieve 23\% overall improvement.

Three topology modes (bounded, toroidal, infinite) support diverse application domains including game development, molecular dynamics, particle simulations, and computational geometry. The header-only implementation requires only the C++20 standard library and targets Boost-quality standards.

\subsection{Boost Submission Status}

The library is in preparation for submission to Boost, pending:
\begin{itemize}
\item Documentation completion (API reference, tutorial, design rationale)
\item Extended platform testing (MSVC, recent Clang versions)
\item Boost build system integration
\item Peer review addressing
\end{itemize}
...
\textbf{Parallel construction.} Multi-threaded grid building using thread-local cell maps with a merge phase could achieve 2-4x speedup on large datasets. Load balancing and merge overhead require careful tuning.

\textbf{Small vector optimization.} Most cells contain 2-5 entities. Inline storage for small buckets would eliminate heap allocations for the common case, improving cache locality and reducing memory allocator overhead by an estimated 10-15\%.
